...
if __name__ == '__main__':
    ...
    seed_torch(3407)
    secret = torch.randn(32, 16, 3, 224, 224).cuda()
    cover = torch.randn(32, 16, 3, 224, 224).cuda()
    ...
    # fh_model = Hinet()
    fh_model = HiDDeN()
    # fh_model.load_state_dict(torch.load(r'E:\research\code\hidehar\hide\wengnet\param\weng_checkpoint_2975.pt'))
    fh_model.eval()
    ...
    # stego = hide_weng(secret, cover, fh_model)
    # stego = hide_hi(secret, cover, fh_model, dwt, iwt)
    stego = hide_hidden(secret, cover, fh_model)
    print(stego.shape)
    time2 = time.time()
    # stego2 = hide_LF_new(secret, cover, dwt, iwt, fh_model)
    time3 = time.time()